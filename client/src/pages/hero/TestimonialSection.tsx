...
const TestimonialSection = ({ data }: { data?: TestimonialsData }) => {
  return (
    <section className="bg-[#e5f7ff] overflow-hidden relative">
      ...
      <h1 className="ml-20 z-10 absolute -left-10 md:left-0 top-10">
        <em className="leading-[1em] text-4xl text-start md:text-6xl font-black md:text-center not-italic">
          {data?.title && <PortableText value={data.title} />}
        </em>
      </h1>
        ...
        <ScrollVelocityContainer className="w-full">
          {data?.rows?.map((row, rowIdx) => (
            <ScrollVelocityRow
              key={rowIdx}
              baseVelocity={1}
              direction={row.direction === "left" ? -1 : 1}
              className="py-4"
            >
              {row.items?.map((t, index) => (
                ...
                >
                  ...
                  <div className="text-wrap flex-1 h-full">
                    <p className="leading-5 text-sm not-first:mt-6 line-clamp-10 text-gray-500">
                      {t.text}
                    </p>
                  </div>
                  ...
                  <div className="flex self-center items-center justify-center w-full absolute right-0 bottom-3">
                    {Array.from({ length: t.stars || 0 }).map((_, i) => (
                      <div key={i}>
                        <StarIcon />
                      </div>
                    ))}
